...
??? note "Seurat Object"
    ...
    **Data Quality Metrics**
    - `Perc_of_mito_genes`: Percentage of mitochondrial gene counts relative to total RNA counts.
    - `Perc_of_ribosomal_genes`: Percentage of ribosomal protein genes (RPL/RPS) relative to total RNA counts.
    - `log10GenesPerUMI`: Log10-transformed ratio of detected genes per UMI, used as a quality indicator.
    ...
    **UCell Enrichment Scores (scGate-Based Annotation)**
    - `<CellType>_UCell`: UCell enrichment scores indicating gene set activity for specific immune or stromal cell types. Examples include:
        - `Immune_UCell`

    **Cell Type Information from scGate**
    - `is.pure_<CellType>`: Boolean classification indicating if a cell belongs to a specific cell type according to scGate. Examples include: e.g. `is.pure_Bcell` 
    - `scGate_multi`: Multi-label classification generated by scGate indicating the main detected cell types for each cell.
    - `CellOntology_name`: Cell ontology name assigned to each cell.
    - `CellOntology_ID`: Corresponding cell ontology ID for the assigned cell type.

    **Annotation Based on Raw Counts**
    - `CD4CD8_BY_EXPRS`: Annotation based on CD4/CD8 gene expression for T cell subtype classification.
    - `CD3_BY_EXPRS`: Annotation based on CD3 gene expression, typically identifying T cells.
    - `CAR_BY_EXPRS`: Annotation of CAR-T cells based on chimeric antigen receptor (CAR) expression (optional).
