...
##### Pipeline Overview
- HANDLE_REFERENCES - Depending on the libraries used (GEX, VDJ, ADT) and the CAR construct (FASTA, GTF), custom references (for CellRanger) are created.
- RUN_SECONDARY_ANALYSIS - Executes CellRanger Multi-analysis and generates a merged, annotated Seurat object as well as CAR specific metrics
- RUN_QUALITY_CONTROL - Quality control is performed using MultiQC and other QC-tools.

![DAG Graph](../../../uploads/DAG_graph.drawio.svg){.half-size}
...
![MulitQC](../../../uploads/MultiQC_Report.png)
...
![CAR__metrics](../../../uploads/CAR-QC-1.png)
...
```bash
git clone https://ribogit.izi.fraunhofer.de/david.schmidt/car-flow.git
```

Before attempting to run the pipeline, ensure that Nextflow is installed and up-to-date (`version >= 24.10.2`).
If you use the pipeline with the Singularity container (recommended via`-profile sigularity`), Singularity must be installed and included in PATH, as well.
...
## Profiles
The `-profile <Profile>` option allows predefined configuration profiles from the `nextflow.config` file to be selected when running the pipeline, altering its execution. For example, the `singularity` profile can be used to run processes within predefined Singularity containers. The `slurm` profile can be used to employ the workload management tool `slurm` for distributing processes in an HPC environment. Profiles can also be combined, allowing the simultaneous application of both the `slurm` and `singularity` profiles by separating the profile by a comma (`-profile singularity,slurm`). By default, it is assumed that all required programs are already in PATH and that the pipeline should be executed locally.

## Running the pipeline
Just like any other pipeline, the base command to launch this pipeline is `nextflow run main.nf`. To use the parameters file, add the `-params-file <...>` option as well as the `-profile <...>` option to fit the execution to your needs.

```bash
nextflow run main.nf -profile <profile(-s)> -params-file <your params-file>
```

## Diving deeper
To get a better understanding of what you can do with the pipeline, check out the documentation
